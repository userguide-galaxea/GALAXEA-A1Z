...
- URDF 模型文件（包内自带，见 `a1z/robot_models/a1z/`，默认使用 `A1Z_2kg.urdf`）
...
```bash
# 零力漂浮（默认 URDF A1Z_2kg.urdf）
python examples/gravity_comp.py

# 使用 nogripper URDF，从小补偿因子开始（推荐首次调试方式）
...
可用 URDF 模型（`a1z/robot_models/a1z/`）：

| 文件 | 说明 |
|------|------|
| `A1Z_2kg.urdf` | 默认，末端挂载 2 kg 负载 |
| `A1Z_nogripper.urdf` | 无夹爪 / 裸法兰 |
| `A1XGEN2_Noumenon.urdf` | A1X Gen2 变体 |
...
| 2 | arm_joint3 | [0°, 200.38°] | [0.000, 3.497] | [0°, 180°] | [0.000, 3.142] |
